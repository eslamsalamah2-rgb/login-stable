...
class InventoryDropWorkerModule:
    ...
    Real drop rule:
      ...
      - click the item once to pick it up
      - wait the user-configured pickup->drop delay
      - click the drop target once
      - confirm Yes
      - capture/scan again before the next item

    Stop rule:
      - key 9 sets the runner stop event
      - this worker checks it before/after scans and between mouse actions
    ...
    def _pickup_to_drop_delay(self):
        return self._float_setting(
            "inventory_drop_pickup_to_drop_delay",
            0.15,
            minimum=0.0,
            maximum=5.0,
        )
    ...
    def _execute_drop(self, action, pid, hwnd, grid):
        ...
        pickup_to_drop_delay = self._pickup_to_drop_delay()
